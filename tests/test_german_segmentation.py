...
def test_german_abbreviations_and_ordinals_do_not_create_false_boundaries():
    ...
    doc = PlainTextDocumentParser().parse(
        text, PipelineConfig(generation=GenerationConfig(lang="de")), Trace()
    )
    assert len(doc.segments) == 2
    assert doc.segments[0].text.endswith("1 Min. ruhen.")
    assert doc.segments[1].text == "Die Kosten liegen bei ca. 12,80 EUR zzgl. Pfand."
