...
def score_phrase(
    ...
    *,
    ...
    for index, text in enumerate(DEMO_SEGMENTS):
        ...
        before_gap = max(0, target_start - before[1]) / SAMPLE_RATE if before is not None else 0.0
        after_gap = max(0, after[0] - target_end) / SAMPLE_RATE if after is not None else 0.0
        if before_gap > MAX_BOUNDARY_GAP_SECONDS or after_gap > MAX_BOUNDARY_GAP_SECONDS:
            continue
